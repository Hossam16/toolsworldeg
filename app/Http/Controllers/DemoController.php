...
class DemoController extends Controller
{
    ...
    public function convert_assets(Request $request){
        ...
        // $business_setting->value = json_encode(getCategories()->where('top', 1)->pluck('id')->toArray());
        // $business_setting->save();
        //
    ...
    public function convert_category(){
        ...
        foreach (SubSubCategory::all() as $key => $value) {
            $category = new Category;
            $parent = Category::find(getCategories()->where('name', SubCategory::find($value->sub_category_id)->name)->first()->id);
        ...
        foreach (Product::all() as $key => $value) {
            try{
                if ($value->subsubcategory_id == null) {
                    $value->category_id = getCategories()->where('name', SubCategory::find($value->subcategory_id)->name)->first()->id;
                    $value->save();
                }
                else {
                    $value->category_id = getCategories()->where('name', SubSubCategory::find($value->subsubcategory_id)->name)->first()->id;
                    $value->save();
                }
            }
            catch(\Exception $e){
