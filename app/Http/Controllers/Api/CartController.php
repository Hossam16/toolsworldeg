...
class CartController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        $tax = 0;
		if($color !=''){
$data=DB::table('colors')->select('name')->where('code', $color)->first();
		$variant=$data->name.'-'.$variant;
		}
	   if ($variant == '' && $color == '')
    ...
    public function changeQuantity(Request $request)
    {
        ...
        if ($cart != null) {
            if ($cart->product->stocks->where('variant', $cart->variation)->first()->qty >= $request->quantity) {
                ...
                return response()->json(['status' => 'Cart updated','message' => 'Cart updated'], 200);
            }
            else {
                return response()->json(['status' => 'Faild','message' => 'Maximum available quantity reached'], 200);
            }
        }

        return response()->json(['status' => 'wrong','message' => 'Something went wrong'], 200);
    }
