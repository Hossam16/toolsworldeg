...
class OrderController extends Controller
{
    public function processOrder(Request $request)
    {
        ...
        if (getBusinessSetting()->where('type', 'shipping_type')->first()->value == 'flat_rate') {
            $shipping = getBusinessSetting()->where('type', 'flat_rate_shipping_cost')->first()->value;
        }
        elseif (getBusinessSetting()->where('type', 'shipping_type')->first()->value == 'seller_wise_shipping') {
            foreach ($cartItems as $cartItem) {
                ...
                if(!empty($admin_products)){
                    $shipping = getBusinessSetting()->where('type', 'shipping_cost_admin')->first()->value;
                }
...
//return ['f7ff'];
        ...
        foreach ($cartItems as $cartItem) {
            ...
            elseif (getBusinessSetting()->where('type', 'shipping_type')->first()->value == 'seller_wise_shipping') {
                if($product->added_by == 'admin'){
                    $order_detail_shipping_cost = getBusinessSetting()->where('type', 'shipping_cost_admin')->first()->value/count($admin_products);
                }
                else {
