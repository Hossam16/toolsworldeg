...
if (! function_exists('format_price')) {
    ...
    {
        if (getBusinessSetting()->where('type', 'decimal_separator')->first()->value == 1) {
...
function getShippingCost($index){
    ...
    if (getBusinessSetting()->where('type', 'shipping_type')->first()->value == 'flat_rate') {
        $calculate_shipping = getBusinessSetting()->where('type', 'flat_rate_shipping_cost')->first()->value;
    }
    elseif (getBusinessSetting()->where('type', 'shipping_type')->first()->value == 'seller_wise_shipping') {
        ...
        if(!empty($admin_products)){
            $calculate_shipping = getBusinessSetting()->where('type', 'shipping_cost_admin')->first()->value;
        }
