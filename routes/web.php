...
Route::get(
	...
	function () {
		...
		$items = Translation::select("lang_key", "lang_value")->where("lang", "en")->get();
		$keyValueArray = [];
		// Iterate over the items and build the array
		foreach ($items as $item) {
			$keyValueArray[$item->lang_key] = $item->lang_value;
		}
		$fileContent = '<?php return ' . var_export($keyValueArray, true) . ';';
		// Path to the translations.php file
		$filePath = resource_path('lang/en/translation.php');
		// Write the data to the file
		file_put_contents($filePath, $fileContent);


		$items = Translation::select("lang_key", "lang_value")->where("lang", "eg")->get();
		$keyValueArray = [];
		// Iterate over the items and build the array
		foreach ($items as $item) {
			$keyValueArray[$item->lang_key] = $item->lang_value;
		...
		$fileContent = '<?php return ' . var_export($keyValueArray, true) . ';';
		// Path to the translations.php file
		$filePath = resource_path('lang/eg/translation.php');
		// Write the data to the file
		file_put_contents($filePath, $fileContent);



		$items = CategoryTranslation::select("category_id", "name")->where("lang", "eg")->get();
		$keyValueArray = [];
		// Iterate over the items and build the array
		foreach ($items as $item) {
		...
		$fileContent = '<?php return ' . var_export($keyValueArray, true) . ';';
		// Path to the translations.php file
		$filePath = resource_path('lang/eg/Category.php');
		// Write the data to the file
		file_put_contents($filePath, $fileContent);
		...
		$fileContent = '<?php return ' . var_export($keyValueArray, true) . ';';
		// Path to the translations.php file
		$filePath = resource_path('lang/en/Category.php');
		// Write the data to the file
		file_put_contents($filePath, $fileContent);
...
Route::get('/searchScroll', 'HomeController@searchScroll')->name('searchScroll');
...
//Stripe END
Route::any('/paymob/payment/callback', 'PAYMOBController@callback')->name('paymob.callback');
Route::get('/paymob/success', 'PAYMOBController@success')->name('paymob.success');
Route::get('/paymob/cancel', 'PAYMOBController@cancel')->name('paymob.cancel');
...
Route::group(['middleware' => ['auth']], function () {
	...
	///
	Route::get('/product-bulk-casheir/index', 'ExternalController@index')->name('product_bulk_update.index');
	Route::post('/bulk-product-casheir_price', 'ExternalController@bulk_upload_price')->name('bulk_product_update_price');
	Route::post('/bulk-product-casheir_stock', 'ExternalController@bulk_upload_stock')->name('bulk_product_update_stock');
