...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6"><strong>{{translate('Upload Product Price List')}}</strong></h5>
        </div>
        <div class="card-body">
            <form class="form-horizontal" action="{{ route('bulk_product_update_price') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group mb-0">
                    <button type="submit" class="btn btn-info">Upload Price (XLSX)</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="card">
        ...
        <div class="card-body">
            <form class="form-horizontal" action="{{ route('bulk_product_update_stock') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group mb-0">
                    <button type="submit" class="btn btn-info">Upload Stock (XLSX)</button>
                </div>
            </form>
        </div>
    </div>
