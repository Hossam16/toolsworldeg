...
<div class="col-lg-8 mx-auto">
	<form class="form form-horizontal mar-top" action="{{route('products.update', $product->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
        ...
        <input type="hidden" name="lang" value="eg">
		@csrf
		<div class="card">
           ...
           <div class="form-group row">
                    <label class="col-lg-3 col-from-label">SKU <i class="las la-language text-danger" title="{{translate('Translatable')}}"></i></label>
                    <div class="col-lg-8">
                        <input type="text" class="form-control" name="sku" placeholder="SKU" value="{{ $product->sku }}" required>
                    
					</div>
					 @if ($errors->has('sku'))
					                 	<span class="help-block small" style="color: #f00">
					                        {{ $errors->first('sku') }}
					                    </span>
					                @endif
                </div>
		...
		@if (getBusinessSetting()->where('type', 'shipping_type')->first()->value == 'product_wise_shipping')
            <div class="card">
				<div class="card-header">
